...
namespace UnrealIncludesFixer
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var proj = Serializer.ReadProjectFile("Intermediate/ProjectFiles/" + projName + ".vcxproj");
            if(proj == null)
            {
                Console.WriteLine("Failed to read Project File\n");
                return;
            }
            ProjectManipulator Manipulator = new ProjectManipulator();

            var includes = Manipulator.ReadNMakeIncludes(proj);

            var propsProj = Manipulator.CreatePropsProject(includes);

            Serializer.WriteProjectFile(propsProj, "Intermediate/ProjectFiles/UnrealIncludes.props");
